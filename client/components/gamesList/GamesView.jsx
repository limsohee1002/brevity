    ...
    return (
        <div>
         ...
         <Switch>
           ...
           {this.state.games.map((game, i) => <Route key={i} path={'/gameList/' + (game.name ? game.name.replace(/ /g,'') : game.name)} render={() => <GameFrame key={i} gameObject={game} user={this.props.user}/>} />)}
         </Switch>
       </div>
    );
  }
